...
extension YTLiveStreaming {
    ...
    public func createBroadcast(_ title: String, description: String?, startTime: Date, completion: @escaping (Result<LiveBroadcastStreamModel, YTError>) -> Void) {
        ...
        YTLiveRequest.createLiveBroadcast(title, startDateTime: startTime, completion: { result in
            switch result {
                case .success(let liveBroadcast):
                    // Create Live stream
                    YTLiveRequest.createLiveStream(title, description: liveStreamDescription, streamName: liveStreamName) { result in
                        switch result {
                            case .success(let liveStream):
                                // Bind live stream
                                YTLiveRequest.bindLiveBroadcast(broadcastId: liveBroadcast.id, liveStreamId: liveStream.id, completion: { result in
                                    switch result {
                                        case .success(let liveBroadcast):
...
extension YTLiveStreaming {
    ...
    func testUpdateLiveStream() {
        ...
        YTLiveRequest.updateLiveStream(liveStreamId, title: title, format: format, ingestionType: ingestionType, completion: { result in
            switch result {
                case .success():
                    print("All right")
                case .failure(let error):
